...
func TestSyncCatalogDatesDownloadsCachesAndAppliesAnnouncement(t *testing.T) {
	...
	if _, err := repository.ApplyCatalogSnapshot(
		ctx,
		[]domain.CatalogGame{{ID: "g1", Title: "Cyberpunk 2077", TitleEn: "Cyberpunk 2077"}},
...
<div class="post-single__footer"></div></div></body></html>`
	...
	service := newDateTestService(t, repository, client)
	first, err := service.Sync(ctx, false, 2022)
	if err != nil {
...
FROM catalog_memberships WHERE game_id = 'g1' AND removed_on IS NULL`,
	...
	second, err := service.Sync(ctx, false, 2022)
	if err != nil {
...
func TestSyncCatalogDatesAppliesCompleteVerifiedBackfill(t *testing.T) {
	...
	repository := sqlite.NewRepository(db)
	games := make([]domain.CatalogGame, 0, len(manifest.Entries)+len(manifest.KeepNull))
	for _, entry := range manifest.Entries {
		games = append(games, domain.CatalogGame{ID: entry.GameID, Title: entry.Title, TitleEn: entry.Title})
	}
	for _, identity := range manifest.KeepNull {
		games = append(games, domain.CatalogGame{ID: identity.GameID, Title: identity.Title, TitleEn: identity.Title})
	}
	if _, err := repository.ApplyCatalogSnapshot(
		ctx,
		games,
...
<sitemap><loc>https://blog.playstation.com/wp-sitemap-posts-recent.xml</loc></sitemap>
	...
	service := newDateTestService(t, repository, client)
	stats, err := service.Sync(ctx, false, 2026)
	if err != nil {
...
WHERE cm.game_id = 'EP4395-CUSA12941_00-FORTHEKING000001'
  AND cm.removed_on IS NULL`).Scan(&forTheKingDate); err != nil {
	...
	second, err := service.Sync(ctx, false, 2026)
	if err != nil {
...
func TestMatchCatalogDateTargetsIgnoresCandidatesTooFarAfterObservedFirstSeen(t *testing.T) {
	...
	result := matchCatalogDateTargets(
		[]domain.CatalogDateTarget{{
			MembershipID: 1,
			Title:        "Cyberpunk 2077",
			TitleEn:      "Cyberpunk 2077",
			FirstSeenAt:  firstSeen,
		}},
		[]domain.CatalogDateCandidate{{
...
func TestRefreshCatalogAnnouncementCacheStopsOnCancelledContext(t *testing.T) {
	...
	service := newDateTestService(t, sqlite.NewRepository(db), client)
	var stats CatalogDateSyncStats
	err = service.refreshCache(ctx, []domain.AnnouncementRef{
...
func TestMatchCatalogDateTargetsIgnoresStaleAnnouncementForReturningGame(t *testing.T) {
	...
	addedOn := time.Date(2023, 1, 17, 0, 0, 0, 0, time.UTC)
	previousRemovedOn := time.Date(2026, 7, 20, 0, 0, 0, 0, time.UTC)
	result := matchCatalogDateTargets(
		[]domain.CatalogDateTarget{{
			MembershipID:      1,
			Title:             "Far Cry 6",
			...
			PreviousRemovedOn: &previousRemovedOn,
		}},
		[]domain.CatalogDateCandidate{{
...
func newDateTestService(t *testing.T, repository *sqlite.Repository, client *http.Client) *CatalogDateService {
	t.Helper()
	service, err := NewCatalogDateService(CatalogDateDependencies{
		Repository:    repository,
		Source:        psstore.NewClient(client),
		BackfillJSON:  mustBackfillJSON(t),
		ParserVersion: psstore.CatalogAnnouncementParserVersion,
		Sleep:         func(context.Context, time.Duration) error { return nil },
	})
	if err != nil {
		t.Fatalf("new date service: %v", err)
	}
	return service
}
...
func (function syncRoundTripFunc) RoundTrip(request *http.Request) (*http.Response, error) {
