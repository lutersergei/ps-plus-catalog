...
func TestHandleIndexOffsetParamOverridesPage(t *testing.T) {
	...
	for i := 1; i <= 30; i++ {
		...
		if err := upsertCatalogTestGame(db, domain.CatalogGame{ID: id, Title: title}); err != nil {
...
func TestInfiniteFeedPrefetchesBeforeFooter(t *testing.T) {
	...
	data := pageData{
		Result: viewListResult{
			Games:      []gameView{{ID: "g1", Title: "Game"}},
			Total:      50,
			Page:       1,
			PageSize:   pageSize,
			TotalPages: 2,
...
func TestFragmentRendersOnlyCardsWithTotalHeader(t *testing.T) {
	...
	for i := 1; i <= 30; i++ {
		id := fmt.Sprintf("g%02d", i)
		if err := upsertCatalogTestGame(db, domain.CatalogGame{ID: id, Title: fmt.Sprintf("G%02d", i)}); err != nil {
...
func TestHandleIndexParsesCriticAndPlayerFilters(t *testing.T) {
	...
	for _, game := range []domain.CatalogGame{
		{ID: "g1", Title: "High Both"},
		{ID: "g2", Title: "Low Player"},
	} {
		if err := upsertCatalogTestGame(db, game); err != nil {
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				ID:                    "g1",
				Title:                 "Game",
				Metacritic:            optionalInt{Int64: 80, Valid: true},
				MetacriticUser:        optionalInt{Int64: 75, Valid: true},
				OpenCritic:            optionalInt{Int64: 82, Valid: true},
				OpenCriticPlayer:      optionalInt{Int64: 78, Valid: true},
				Average:               optionalFloat{Float64: 79, Valid: true},
				CriticAverage:         optionalFloat{Float64: 81, Valid: true},
				PlayerAverage:         optionalFloat{Float64: 76.5, Valid: true},
				MetacriticUserCount:   optionalInt{Int64: 1900, Valid: true},
				OpenCriticPlayerCount: optionalInt{Int64: 57, Valid: true},
			}},
		},
	}
...
func TestIndexTemplateRendersSidebarFilterLayout(t *testing.T) {
	...
	data := pageData{
		...
		Params: domain.ListParams{Sort: "reviews", Order: "desc"},
	}
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				ID:                 "g1",
				Title:              "Game",
				CatalogAddedOn:     optionalTime{Time: time.Date(2026, 7, 21, 0, 0, 0, 0, time.UTC), Valid: true},
				CatalogAddedSource: optionalString{String: "announcement", Valid: true},
				CatalogSourceURL:   optionalString{String: "https://blog.playstation.com/example/", Valid: true},
			}},
			Page:     1,
			PageSize: pageSize,
		},
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				ID:                 "g1",
				Title:              "Game",
				CatalogAddedOn:     optionalTime{Time: time.Date(2022, 6, 23, 0, 0, 0, 0, time.UTC), Valid: true},
				CatalogAddedSource: optionalString{String: "verified", Valid: true},
				CatalogSourceURL:   optionalString{String: "https://example.com/history", Valid: true},
			}},
			Page:     1,
			PageSize: pageSize,
		},
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				ID:                 "g2",
				Title:              "Some Game",
				CatalogAddedOn:     optionalTime{Time: added, Valid: true},
				CatalogAddedSource: optionalString{String: "observed", Valid: true},
			}},
...
func TestIndexTemplateRendersNoAddedDateBadge(t *testing.T) {
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{ID: "g3", Title: "No Date Game"}},
			Total: 1, Page: 1, PageSize: pageSize, TotalPages: 1,
		},
	}
	var buf bytes.Buffer
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				...
				Title:                 "Game",
				OpenCriticPlayer:      optionalInt{Int64: 78, Valid: true},
				OpenCriticPlayerCount: optionalInt{Int64: 11, Valid: true},
			}},
		},
	}
...
func TestIndexTemplateRendersLetterIndexAndMoreLink(t *testing.T) {
	...
	data := pageData{
		...
		Params:     domain.ListParams{Sort: "title"},
		BaseQuery:  template.URL("sort=title&order=asc"),
		Buckets:    []domain.IndexBucket{{Label: "#", Offset: 0}, {Label: "A", Offset: 3}},
		NextOffset: pageSize,
		HasNext:    true,
	}
	var buf bytes.Buffer
...
func TestIndexTemplateHidesLetterIndexWithoutBuckets(t *testing.T) {
	...
	data := pageData{
		Result: viewListResult{Games: []gameView{{ID: "g1", Title: "Game"}}},
		Params: domain.ListParams{Sort: "player"},
	}
	var buf bytes.Buffer
	if err := tmpl.Execute(&buf, data); err != nil {
		t.Fatalf("execute: %v", err)
...
func TestHandleIndexComputesBucketsPerSort(t *testing.T) {
	...
	defer db.Close()
	if err := upsertCatalogTestGame(db, domain.CatalogGame{ID: "g1", Title: "Alpha", ReleaseYear: 2020}); err != nil {
		t.Fatalf("upsert: %v", err)
	}
...
func TestFmtCount(t *testing.T) {
	...
	for _, c := range cases {
		if got := formatCount(c.n); got != c.want {
			t.Errorf("formatCount(%d) = %q, ждали %q", c.n, got, c.want)
		}
	}
}

func TestHLTBBarScale(t *testing.T) {
	if got := hltbPercent(42); got != 70 {
		t.Errorf("hltbPercent(42) = %d, ждали 70", got)
	}
	if got := hltbPercent(91); got != 100 {
		t.Errorf("hltbPercent(91) = %d, ждали 100 (клампится к шкале)", got)
	}
	if got := hltbPercent(0); got != 0 {
		t.Errorf("hltbPercent(0) = %d, ждали 0", got)
	}
	if !hltbOver(91) {
...
func TestHandleIndexIgnoresSliderEdgeValues(t *testing.T) {
	...
	if err := upsertCatalogTestGame(db, domain.CatalogGame{ID: "g1", Title: "No Scores"}); err != nil {
		t.Fatalf("upsert: %v", err)
	}
	...
	data := pageData{
		Result: viewListResult{
			Games: []gameView{{
				...
				CatalogAddedOn:     optionalTime{Time: added, Valid: true},
				CatalogAddedSource: optionalString{String: "announcement", Valid: true},
				CatalogSourceURL:   optionalString{String: "https://example.com/ann", Valid: true},
			}},
			Total: 1, Page: 1, PageSize: pageSize, TotalPages: 1,
		},
	}
...
func newIndexTemplate() (*template.Template, error) {
	...
	handler, err := NewCatalogHandler(string(raw), nil, nil)
	if err != nil {
		return nil, err
	}
	return handler.template, nil
}

func handleIndex(response *httptest.ResponseRecorder, request *http.Request, db *sql.DB, tmpl *template.Template) {
	logger := slog.New(slog.NewTextHandler(io.Discard, nil))
	handler := &CatalogHandler{
		browser:  services.NewCatalogService(sqlite.NewRepository(db)),
		template: tmpl,
		logger:   logger,
	}
...
func upsertCatalogTestGame(db *sql.DB, game domain.CatalogGame) error {
	_, err := db.Exec(`
...
ON CONFLICT(id) DO UPDATE SET
  ...
  active=1`,
		game.ID, game.Title, game.TitleEn, game.ReleaseYear,
		strings.Join(game.Platforms, ", "), game.ImageURL, game.StoreURL,
	)
	return err
}
