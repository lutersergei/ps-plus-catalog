...
func TestMatchCatalogAdditionNormalizesStoreEditionAndChoosesLatestReturn(t *testing.T) {
	candidates := []CatalogAddition{
		...
		{Title: "Far Cry 6", AddedOn: dateForTest("2026-07-21")},
	}
	got, found, ambiguous := MatchCatalogAddition(
		"FARCRY 6 Standard Edition PS4 & PS5",
...
func TestMatchCatalogAdditionExactTitleBeatsSimilarSequel(t *testing.T) {
	candidates := []CatalogAddition{
		{Title: "Cat Quest", AddedOn: dateForTest("2024-07-16")},
		{Title: "Cat Quest II", AddedOn: dateForTest("2024-07-16")},
		{Title: "Cat Quest III", AddedOn: dateForTest("2025-08-19")},
	}
	got, found, ambiguous := MatchCatalogAddition("Cat Quest", "Cat Quest", candidates)
	if !found || ambiguous || got.Title != "Cat Quest" {
		t.Fatalf("got=%+v found=%v ambiguous=%v", got, found, ambiguous)
	}
}

func TestMatchCatalogAdditionDoesNotMatchTitlePrefixToSequel(t *testing.T) {
	got, found, ambiguous := MatchCatalogAddition("Cat Quest", "Cat Quest", []CatalogAddition{{
		Title: "Cat Quest II", AddedOn: dateForTest("2025-08-19"),
	}})
	if found || ambiguous {
		t.Fatalf("got=%+v found=%v ambiguous=%v; sequel must not be treated as the original", got, found, ambiguous)
	}
}
