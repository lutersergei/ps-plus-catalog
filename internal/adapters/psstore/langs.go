...
// FetchLanguages получает коды языков озвучки и субтитров для игры со страницы PS Store.
...
func (c *Client) FetchLanguages(ctx context.Context, conceptURL string) (spoken, screen []string, err error) {
	...
	resp, err := doTrustedRequest(c.http, req, "store.playstation.com")
	if err != nil {
		return nil, nil, err
	}
