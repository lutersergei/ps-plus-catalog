...
// Client — HTTP-адаптер PlayStation. Зависимость от HTTP-клиента явно передаётся
// в точке композиции и легко заменяется в тестах сервисов.
type Client struct {
	http *http.Client
}

// NewClient создаёт адаптер PlayStation поверх переданного HTTP-клиента.
func NewClient(client *http.Client) *Client {
	return &Client{http: client}
}

// FetchCatalog загружает и разбирает полный каталог PS Plus Extra региона TR.
func (c *Client) FetchCatalog(ctx context.Context) ([]Game, error) {
	...
	resp, err := doTrustedRequest(c.http, req, "www.playstation.com")
	if err != nil {
