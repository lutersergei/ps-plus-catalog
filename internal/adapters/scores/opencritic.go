...
const (
	...
	ocSiteHost = "api.opencritic.com"
)

var errAllKeysExhausted = fmt.Errorf("opencritic: %w", domain.ErrProviderQuotaExhausted)
...
type OpenCriticResult = domain.OpenCriticResult
type OpenCriticGenre = domain.OpenCriticGenre

// OpenCriticClient объединяет пул ключей и оба HTTP API OpenCritic.
...
func NewOpenCriticClient(client *http.Client, keys []string, siteKey string) *OpenCriticClient {
	return &OpenCriticClient{http: client, pool: NewKeyPool(keys), siteKey: siteKey}
}

func (c *OpenCriticClient) Enabled() bool { return !c.pool.Empty() }
func (c *OpenCriticClient) KeyCount() int { return c.pool.Count() }

func (c *OpenCriticClient) Lookup(ctx context.Context, title string) (OpenCriticResult, error) {
	return openCriticScores(ctx, c.http, c.pool, c.siteKey, title)
}
...
// parseOpenCriticGameResult достаёт topCriticScore из ответа /game/<id>.
...
func openCriticPlayerRating(ctx context.Context, c *http.Client, siteKey string, gameID int) (Rating, error) {
	...
	resp, err := doTrustedRequest(c, req, ocSiteHost)
	if err != nil {
...
func ocDo(ctx context.Context, c *http.Client, key, path string) ([]byte, int, error) {
	...
	resp, err := doTrustedRequest(c, req, ocHost)
	if err != nil {
		return nil, 0, err
	}
	defer resp.Body.Close()
