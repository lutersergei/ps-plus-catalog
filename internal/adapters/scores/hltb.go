...
type HLTBResult = domain.HLTBResult

// HLTBClient держит honeypot-токен поиска и переиспользует его между запросами,
...
// NewHLTBClient создаёт сессию поверх http-клиента.
func NewHLTBClient(c *http.Client) *HLTBClient { return &HLTBClient{client: c} }
...
func (s *HLTBClient) Lookup(ctx context.Context, title string) (res HLTBResult, found, conclusive bool, err error) {
	sawResults := false
...
func (s *HLTBClient) searchWithRetry(ctx context.Context, terms []string) ([]hltbGame, error) {
...
func (s *HLTBClient) init(ctx context.Context) error {
	...
	resp, err := doTrustedRequest(s.client, req, "howlongtobeat.com", "www.howlongtobeat.com")
	if err != nil {
		return fmt.Errorf("hltb init: %w", err)
	}
...
func (s *HLTBClient) search(ctx context.Context, terms []string) ([]hltbGame, error) {
	if s.token == "" {
...
func (s *HLTBClient) setBrowserHeaders(req *http.Request) {
