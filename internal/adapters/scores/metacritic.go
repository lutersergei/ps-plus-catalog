...
const (
	metacriticWebHost     = "www.metacritic.com"
	metacriticBackendHost = "backend.metacritic.com"
)
...
type Rating = domain.Rating
type MetacriticResult = domain.MetacriticResult

// MetacriticClient — HTTP-адаптер Metacritic.
type MetacriticClient struct {
	http *http.Client
}

func NewMetacriticClient(client *http.Client) *MetacriticClient {
	return &MetacriticClient{http: client}
}

// Lookup возвращает оценки критиков и игроков для английского названия игры.
func (c *MetacriticClient) Lookup(ctx context.Context, titleEn string) (MetacriticResult, error) {
	return metacriticScores(ctx, c.http, titleEn)
}

type metacriticPage struct {
...
func metacriticScores(ctx context.Context, c *http.Client, titleEn string) (MetacriticResult, error) {
	...
	for _, slug := range metacriticSlugCandidates(titleEn) {
		seen[slug] = true
		page, found, err := metacriticScoresBySlug(ctx, c, slug)
		if err != nil {
		...
		if page.title != "" && !metacriticTitlesMatch(titleEn, page.title) {
			continue
		}
		res := page.result
		metacriticFillUserScore(ctx, c, slug, page.html, &res)
		return res, nil
	}
	...
	for _, slug := range searchSlugs {
		...
		if !found || !metacriticTitlesMatch(titleEn, page.title) {
			continue
		}
		res := page.result
		metacriticFillUserScore(ctx, c, slug, page.html, &res)
...
func metacriticScoresBySlug(ctx context.Context, c *http.Client, slug string) (metacriticPage, bool, error) {
	pageURL := "https://www.metacritic.com/game/" + slug + "/"
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, pageURL, nil)
	if err != nil {
		return metacriticPage{}, false, err
	}
	req.Header.Set("User-Agent", mcUserAgent)
	req.Header.Set("Accept-Language", "en-US,en;q=0.9")

	resp, err := doTrustedRequest(c, req, metacriticWebHost)
	if err != nil {
		return metacriticPage{}, false, fmt.Errorf("metacritic fetch: %w", err)
	}
	defer resp.Body.Close()
	if resp.StatusCode == http.StatusNotFound {
		return metacriticPage{}, false, nil // игры нет под таким slug
	}
	if resp.StatusCode != http.StatusOK {
		return metacriticPage{}, false, fmt.Errorf("metacritic status %d", resp.StatusCode)
	}
	body, err := readLimited(resp.Body, maxHTMLBytes)
	if err != nil {
		return metacriticPage{}, false, fmt.Errorf("metacritic body: %w", err)
	}
	score, found, err := parseMetacritic(body)
	if err != nil {
		return metacriticPage{}, false, err
	}
	res := MetacriticResult{PageURL: pageURL, Genres: parseMetacriticGenres(body)}
	if found {
		res.Critic = Rating{Score: score, Found: true}
	}
	return metacriticPage{result: res, title: parseMetacriticTitle(body), html: body}, true, nil
}
...
func metacriticUserScore(ctx context.Context, c *http.Client, url, pageSlug string) (Rating, error) {
	...
	resp, err := doTrustedRequest(c, req, metacriticBackendHost)
	if err != nil {
