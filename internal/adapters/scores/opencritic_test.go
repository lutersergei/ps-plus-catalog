...
func TestParseOpenCriticGame(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			result, err := parseOpenCriticGameResult([]byte(c.raw))
			if (err != nil) != c.wantErr {
				t.Fatalf("err=%v, wantErr=%v", err, c.wantErr)
			}
			if result.Found != c.wantFound {
				t.Fatalf("found=%v, ждали %v", result.Found, c.wantFound)
			}
			if result.Found && result.Score != c.wantScore {
				t.Errorf("score=%d, ждали %d", result.Score, c.wantScore)
			}
			if result.Found && result.PageURL != "" {
				t.Errorf("url=%q, ждали пустой url", result.PageURL)
			}
		})
	}
}
...
func TestParseOpenCriticGameURL(t *testing.T) {
	result, err := parseOpenCriticGameResult([]byte(`{
		"topCriticScore": 84.6,
	...
	if !result.Found || result.Score != 85 {
		t.Fatalf("score=%d found=%v, ждали 85 true", result.Score, result.Found)
	}
	if result.PageURL != "https://opencritic.com/game/4503/assassins-creed-origins" {
		t.Fatalf("url=%q", result.PageURL)
	}
}
