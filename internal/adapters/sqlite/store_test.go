...
func TestListGamesClampsHugePageForEmptyCatalog(t *testing.T) {
	db, err := openTestDB(filepath.Join(t.TempDir(), "empty.db"))
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	defer db.Close()

	result, err := ListGames(db, ListParams{Page: int(^uint(0) >> 1), PageSize: 25})
	if err != nil {
		t.Fatalf("list empty catalog: %v", err)
	}
	if result.Page != 1 || result.Total != 0 || len(result.Games) != 0 {
		t.Fatalf("result=%+v, ждали пустую первую страницу", result)
	}
}
...
func TestListGamesSortsByCatalogAddedDateWithUnknownLast(t *testing.T) {
	...
	if !res.Games[0].CatalogAddedOn.Valid || res.Games[0].CatalogAddedOn.Time.Format("02.01.2006") != "21.07.2026" {
		t.Fatalf("added date=%v, ждали 21.07.2026", res.Games[0].CatalogAddedOn)
	}
