...
type catalogDateBackfillMatchResult struct {
	Matches             []store.CatalogDateBackfillMatch
	KeepNullIDs         []int64
	KeepNullGames       []string
	AnnouncementTargets []store.CatalogDateTarget
}

func loadCatalogDateBackfill() (catalogDateBackfill, error) {
	var document catalogDateBackfillDocument
	decoder := json.NewDecoder(bytes.NewReader(catalogDateBackfillJSON))
...
func catalogDateIdentityKey(title string) string {
	return psstore.NormalizeCatalogTitle(title)
}

func (manifest catalogDateBackfill) entryForTarget(target store.CatalogDateTarget) (catalogDateBackfillEntry, bool, error) {
...
func (manifest catalogDateBackfill) entryForTargetTitle(target store.CatalogDateTarget) (catalogDateBackfillEntry, bool, error) {
...
func (manifest catalogDateBackfill) keepNullForTarget(target store.CatalogDateTarget) (catalogDateIdentity, bool, error) {
...
func catalogDateTargetHasTitle(target store.CatalogDateTarget, expected string) bool {
...
// matchCatalogDateBackfillTargets applies curated history only to memberships
// from the baseline snapshot. Later returns must still be dated by a matching
// announcement inside their observation window.
func matchCatalogDateBackfillTargets(
	...
	result := catalogDateBackfillMatchResult{
		Matches:             make([]store.CatalogDateBackfillMatch, 0, len(manifest.Entries)),
		AnnouncementTargets: make([]store.CatalogDateTarget, 0, len(targets)),
	}
	...
	for _, target := range targets {
		...
		if found {
			...
			result.Matches = append(result.Matches, store.CatalogDateBackfillMatch{
